# mainsail-plugin: instructions for AI assistants
...
A solo Bespok3d plugin repo: it ships one plugin, `mainsail` (Mainsail web interface alongside Fluidd,
on your chosen port), and publishes a single index atom into `Bespok3d/main-index/atoms/`. The plugin
carries config, assets, and shell only (Klipper config, an nginx config, the built Mainsail web assets,
and a fetch shell script), no Python.
